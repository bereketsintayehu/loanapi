...
func SendActivationEmail(email, token string) error {
	...
	m.SetBody("text/html", fmt.Sprintf("Please click <a href=\"http://%s%s/users/verify-email/%s/%s\">here</a> to activate you account.", config.EnvConfigs.ServerURL, config.EnvConfigs.ServerPort, token, email))
...
func SendResetLink(email, token string) error {
	...
	m.SetBody("text/html", fmt.Sprintf("Click <a href=\"http://http://%s%s/users/password-update-form/%s\">here</a> to reset your password. Your token is %s", config.EnvConfigs.ServerURL, config.EnvConfigs.ServerPort, token, token))
