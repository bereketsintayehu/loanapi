...
var (
	...
	ErrLoanNotFound        = New("loan not found", http.StatusNotFound)
	ErrLoanCreationFailed  = New("failed to create loan", http.StatusInternalServerError)
	ErrLoanUpdateFailed    = New("failed to update loan", http.StatusInternalServerError)
	ErrLoanDeletionFailed  = New("failed to delete loan", http.StatusInternalServerError)
	ErrFetchingLogsFailed  = New("failed to fetch logs", http.StatusInternalServerError)
	ErrLogCreationFailed   = New("failed to create log", http.StatusInternalServerError)
	ErrInvalidLoanID = New("invalid loan id", http.StatusBadRequest)
	ErrFetchingLoanStatusFailed = New("failed to fetch loan status", http.StatusInternalServerError)
	ErrFetchingLoansFailed = New("failed to fetch loan", http.StatusInternalServerError)
	ErrLoanRejectionFailed = New("failed to reject loan", http.StatusInternalServerError)
	ErrLoanApprovalFailed = New("failed to approve loan", http.StatusInternalServerError)
	ErrOperationFailed = New("operation failed", http.StatusInternalServerError)
	ErrFailedToUpdateLoanStatus = New("failed to update loan status", http.StatusInternalServerError)
)
